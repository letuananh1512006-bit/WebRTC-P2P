...
async function startMeeting(
  ...
  const mesh = new MeshPeerManager(
    signaling,
    currentRoomId,
    (peerId, stream) => attachRemoteVideo(gallery, peerId, stream),
    (peerId) => {
...
function hideMockTiles(): void {
  document.querySelectorAll(".mock-remote-tile").forEach((el) => {
    el.classList.add("hidden");
  });
}

function attachRemoteVideo(gallery: HTMLElement, peerId: string, stream: MediaStream): void {
  const existing = document.getElementById(`video-${peerId}`) as HTMLVideoElement | null;
  if (existing) {
...
function bindMediaButtons(stream: MediaStream): void {
...
function bindLeave(cleanup: () => void): void {
  window.addEventListener("beforeunload", cleanup);
}

function bindCopy(roomId: string): void {
